...
from supernet_backbone.lib_back.utils.helpers import load_checkpoint
try:
...
class Blocks(nn.Module):
    def __init__(self, supernet):
        ...
        self.conv_stem = supernet.conv_stem
        self.bn1 = supernet.bn1
        self.act1 = supernet.act1
        self.blocks = None
...
def load_path_from_supernet(path_model, path_backbone, logger):

    supernet = models.__dict__['LightTrackM_Supernet'](search_size=256, stride=16)

    full_ckpt_path = './checkpoints/supernet/model_best.pth.tar'
    logger.info(f"\nload supernet checkpoint from: {full_ckpt_path}")
    full_state_dict = torch.load(full_ckpt_path, map_location='cpu')['state_dict']

    try:
        supernet.features.load_state_dict(full_state_dict, strict=False)
    except Exception as e:
        logger.info(f'supernet checkpoint not loaded')
        exit(0)

    logger.info(f'\ncompute backbone architecture')
    

    cand_dict = name2path_ablation(path_backbone, sta_num=supernet.sta_num)
    cand_b, head_cand, oup_pos = cand_dict['back'], cand_dict['head'], cand_dict['ops']

    logger.info(f'back_cand: {cand_b}')
    logger.info(f'head_cand: {head_cand}')
    logger.info(f'oup pos: {oup_pos}')

    backbone_index = oup_pos

    blocks = Blocks(supernet.features)
    block_model = supernet.features.build_model(blocks, cand_b, stride=16, backbone_index=backbone_index)

    #path_model = models.__dict__['LightTrackM_Subnet'](path_backbone,stride=16)
    try:
        #path_model.features.load_state_dict(block_model.state_dict(), strict=False)
        path_model.load_state_dict(block_model.state_dict(), strict=False)
        logger.info('backbone initialized successfully')
    except Exception as e:
        logger.info(f'ERROR WHILE LOADING THE BLOCK STATE DICT')
        exit(0)

    return path_model

def load_lighttrack_model(model, model_name: str, checkpoints: str = './checkpoints/', checkpoint_epoch=None, strict=True):
    ...
    if checkpoint_epoch is None:
        ...
        checkpoint_list = sorted(Path(checkpoint_dir).glob('*.pth'), key=os.path.getmtime)
        ...
        print(f'checkpoint epoch provided: {checkpoint_epoch}')
        checkpoint_path = os.path.join(checkpoint_dir, f'checkpoint_e{str(checkpoint_epoch)}.pth')
...
def get_subwindow_tracking_black(im, pos, target_sz, model_sz, original_sz, out_mode='torch'):
    """
    SiamFC type cropping
    """
    # crop_info = dict()
    ...
    if isinstance(pos, float):
        pos = [pos, pos]
    w, h = target_sz.astype(np.int)
    cx, cy = pos.astype(np.int)
    sz = int(original_sz)

    c = int((original_sz+1) / 2)

    im_patch_original = np.zeros((sz,sz,3))
    im_patch_original[int(c-h/2):int(c-h/2)+h,int(c-w/2):int(c-w/2)+w,:] = \
        im[int(cy-h/2):int(cy-h/2)+h,int(cx-w/2):int(cx-w/2)+w,:]
    # vis = False
    # if vis:
    #     cur_t = time.time() % 3600
    #     im_patch_original_bgr = im_patch_original[:,:,::-1].copy()
    #     cv2.imwrite('/home/masterbin-iiau/test/AutoTrack/debug/%d.jpg'%cur_t,im_patch_original_bgr)
    if not np.array_equal(model_sz, original_sz):
        im_patch = cv2.resize(im_patch_original, (model_sz, model_sz))
    else:
        im_patch = im_patch_original
    ...
    if out_mode == "torch":
        return im_to_torch(im_patch.copy())
    else:
        return im_patch
...
def load_pretrain(model, pretrained_path, print_unuse=True, device=None):
    ...
    pretrained_dict = torch.load(pretrained_path, map_location=torch.device('cpu')) #, map_location=lambda storage, loc: storage.cuda(device))
...
def _tlbr2xywh(bbox):
    ...
    '''
    takes a bounding box in the format top left, bottom right

    returns: bounding box in the format top left, width, height
    '''
...
def _tensor2BBox(x):
    '''
    takes a tensor and returns a namedtuple BBox
    '''

    return BBox(x[0].item(), x[1].item(), x[2].item(), x[3].item())

def visualize_bbox(img, bbox=None, points=None, hor_line=None, vert_line=None, path='./plots', subdir: str = None, name: str=None):
    '''
    Visualize a bounding box within an image.

    The bounding box is assumed to be in [x y w h] format.
    '''
    ...
    if hor_line is not None: 
        for hl in hor_line:
            ax1.axhline(y=hl, xmin=0, xmax=511)

    if vert_line is not None: 
        for vl in vert_line:
            ax1.axvline(x=vl, ymin=0, ymax=511)

        

    plt.savefig(os.path.join(img_path, name+'.png'))
    plt.close()
...
def aug_apply(bbox, param, shape, inv=False, rd=False):
    ...
    if not inv:
        ...
        return center2corner(center), None
...
def restore_from(model, optimizer, ckpt_path, restore_optimizer=True):
...
def restore_reid_from_siamese(model, ckpt_path):
    print('restore from {}'.format(ckpt_path))
    device = torch.cuda.current_device()
    ckpt = torch.load(ckpt_path, map_location = lambda storage, loc: storage.cuda(device))
    epoch = ckpt['epoch']
...
def create_logger(cfg, modelFlag='ET', phase='train'):
...
def save_tracking_model(model, epoch, optimizer, model_name, cfg, isbest=False, ema=False):
    pass

def save_model(model, epoch, optimizer, lr_scheduler, model_name, cfg,
               isbest=False, ema=False):
    """
    save model
    """
    if not exists(cfg.CHECKPOINT_DIR):
        os.makedirs(cfg.CHECKPOINT_DIR)
    print('saving checkpoint to ',cfg.CHECKPOINT_DIR)
    if epoch > 0:
        # save_checkpoint({
        #     'epoch': epoch + 1,
        #     'arch': model_name,
        ...
        if not ema:
            ...
            save_checkpoint(states={
                            ...
                            filename='checkpoint_e%d.pth' % (epoch + 1))
        else:
            '''only save model's state_dict'''
            save_checkpoint({
                'state_dict': model.module.state_dict()
            }, isbest, cfg.CHECKPOINT_DIR, 'checkpoint_ema_e%d.pth' % (epoch + 1))
    else:
        print('epoch not save(<1)')
...
def extract_eaos(lines):
    ...
    for line in lines:
        ...
        if (not line.startswith('| ET')) and (not line.startswith('| Baseline')):
            continue
        temp = line.split('|')
...
def _build_warm_up_scheduler(optimizer, cfg, epochs=50, last_epoch=-1, modelFLAG='ET'):
    #cfg = cfg[modelFLAG]
...
class EpsilonScheduler:
    ...
    def __init__(self, start_epoch, end_epoch, num_gpu, batch_size, ds_len=320000):
        '''
        - start_epoch: epoch when to start decreasing epsilon
        - end_epoch: epoch when epsilon should be 0
        '''
        ...
        self.steps_per_epoch = int(self.ds_len / (self.batch_size * self.num_gpu))

        self.start_step = self.start_epoch * self.steps_per_epoch
        self.end_step = self.end_epoch * self.steps_per_epoch

        print(f'eps scheduler start step: {self.start_step}')
        print(f'eps scheduler end step: {self.end_step}')

        self.delta_x = int(self.end_step - self.start_step) 
        self.m = -1 / self.delta_x
        self.q = 1 - (self.m * self.start_step)

    def get_eps(self, step):

        #eps = max(min(1.0, self.m * step + self.q), 0)
        if step <= self.start_step:
            eps=1.0
        elif self.start_step < step <= self.end_step:
            eps = self.m * step + self.q
        else:
            eps = 0
...
class TemperatureScheduler:
    ...
    def __init__(self, start_epoch, num_gpu, batch_size, ds_len=320000, use_early_stop=True, total_epochs=50, early_stop_epoch=30, end_temp=0.01):
        '''
        - start_epoch: epoch when to start decreasing epsilon 
        ...
        - use_early_stop: 
            - true: stop at early_stop_epoch
            - false: stop at total_epochs
        - total_epochs: number of epoochs during training
        - early_stop_epoch: early stop epoch
        '''
        ...
        self.end_epoch = early_stop_epoch if use_early_stop else total_epochs
        ...
        print(f'temp scheduler start step: {self.start_step}')
        print(f'temp scheduler end step: {self.end_step}')

        self.delta_x = int(self.end_step - self.start_step) 
        self.delta_y = 1.0 - self.end_temp
...
class DynamicTemperatureScheduler:
    ...
    def __init__(self, start_epoch, end_epoch, num_gpu=3, batch_size=32, ds_len=500000, start_temp=30.0, end_temp=1.0):
        '''
        - start_epoch: epoch when to start decreasing epsilon 
        ...
        - use_early_stop: 
        ...
        print(f'temp scheduler steps per epoch for {self.num_gpu} gpus: {self.steps_per_epoch}')
        print(f'temp scheduler start step: {self.start_step}')
        print(f'temp scheduler end step: {self.end_step}')

        self.delta_x = int(self.end_step - self.start_step) 
        self.delta_y = self.start_temp - self.end_temp
        self.m = -self.delta_y / self.delta_x
        # y = mx+q  <=> q = y - mx
        self.q = self.start_temp - (self.m * self.start_step)
...
def check_dynamic_temp_scheduler():

    tmp_sched = DynamicTemperatureScheduler(start_epoch=3, end_epoch=39, start_temp=30, end_temp=1.0, num_gpu=3, batch_size=32) 
    
    temp=[]
    x=[]
    for i in range(0,300000,5):
        x.append(i)
        temp.append(tmp_sched.get_temp(i))

    import matplotlib.pyplot as plt
    plt.plot(x,temp)
    plt.ylim(0,40)
    #plt.vlines(x=16665, ymin=0, ymax=2,color="red")
    #plt.vlines(x=99990, ymin=0, ymax=2,color="red")
    plt.savefig('./dynamic_temp_scheduler.png')
    plt.close()

def check_scheduler():

    tmp_sched = TemperatureScheduler(start_epoch=5, num_gpu=3, batch_size=32, end_temp=0.01)
    eps_sched = EpsilonScheduler(start_epoch=1.25, 
                                 end_epoch=5, 
                                 num_gpu=3, 
                                 batch_size=32)
    ...
    for i in range(0,100000,5):
        x.append(i)
        eps.append(eps_sched.get_eps(i))
        temp.append(tmp_sched.get_temp(i))
